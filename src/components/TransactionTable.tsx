...
export default function TransactionTable({}: TransactionTableProps) {
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelected = monthlyDividends.map((n) => n.id);
  ...
  const emptyRows =
    page > 0
      ? Math.max(0, (1 + page) * rowsPerPage - monthlyDividends.length)
      : 0;

  const visibleRows = React.useMemo(() => {
    const sortedMonthlyDividends = [...monthlyDividends].sort((a, b) =>
      compareDesc(parseISO(a.date), parseISO(b.date))
    );

    return sortedMonthlyDividends.slice(
      page * rowsPerPage,
